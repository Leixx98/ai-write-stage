...
全自动 AI 长篇小说创作引擎。确定性引擎跑完整本书，模型在每个需要判断的位置被精确使用：Engine 按事实路由驱动 Architect / Chapter Planner / Writer / Editor，语义裁定按需唤醒 Arbiter。从一句话需求到完整小说，全程无需人工干预。
...
- **确定性引擎 + 异构模型流水线** — Engine 按事实决策表调度 Architect / Chapter Planner / Writer / Editor；复杂规划可用云端模型，批量正文可用本地模型
- **语义裁定可审计** — 选规划师、干预分诊、失败出路等判断由 Arbiter 单次调用完成，每次裁定落盘可回放。越简单越稳定，拒绝复杂编排
- **Step 级断点恢复** — 每个工具执行成功后写入 checkpoint，崩溃后精确到 plan/draft_unit/draft/check/commit 步骤级恢复
...
核心设计：**事实层确定，语义层自主**。可枚举的状态迁移由确定性代码执行（Engine + Route）；边界清晰的判断按需咨询 LLM 函数（Arbiter）；开放式创作交给有界 LLM Worker。一句话概括：一个串行确定性 Engine、四类 Worker、少数几个按需 Arbiter 函数、一个文件系统事实层。
...
└───┬────────┬──────────┬────────┬───────────┘
    │        │          │        │
┌───▼────┐ ┌─▼────────┐ ┌▼──────┐ ┌▼──────┐ ┌▼───────┐
│Architect│ │Chapter   │ │Writer │ │Editor │ │Arbiter │
│(LLM循环)│ │Planner   │ │(LLM循环)│ │(LLM循环)│ │(LLM函数)│
└───┬────┘ └────┬─────┘ └──┬────┘ └──┬────┘ └────────┘
    └───────────┴───────────┴─────────┘
                    │ 工具调用（IO + checkpoint）
┌───────────────▼─────────────────────────────────┐
...
- **Workers** — Architect / Chapter Planner / Writer / Editor 通过 Store 工件协作；章节计划与正文生成可独立配置模型
...
| **Chapter Planner** | 按 Writer 上下文窗口把当前大纲章拆成场景卡和动态字数的写作片段卡 | `novel_context` `read_chapter` `plan_chapter` |
| **Writer** | 按片段卡完成正文、自审和提交 | `novel_context` `read_chapter` `write_chapter_unit` `draft_chapter` `check_consistency` `commit_chapter` |
| **Editor** | 阅读原文，从结构和审美两个层面审阅 | `novel_context` `read_chapter` `save_review` `save_arc_summary` `save_volume_summary` |

### 写作流程

```
用户需求 → Arbiter 选规划师 → Architect 规划骨架+首弧 → Chapter Planner 规划当前章 → Writer 逐片段写作 → Editor 弧级评审
              (裁定落盘)                                     ↑                   │
                                                            ├── 重写/打磨 ◄──────┘
                                                            │
                                                     Architect 展开下一弧/卷
...
2. Writer 调用无参数 `novel_context()`；宿主强制绑定当前章节并只投影当前 unit，Writer 每次仅向 `write_chapter_unit` 提交 `content`，章节号和 unit ID 均由宿主注入。
3. 每个 unit 独立落盘，代码按计划顺序幂等重建整章草稿；中断恢复不会重复追加正文。
4. 全部 unit 完成后 Writer 回读草稿、调用 `check_consistency`，最后以 `commit_chapter` 提交。
...
- **规划与写作解耦** — Chapter Planner 生成场景/片段卡，Writer 按 unit → check → commit 完成正文；复杂规划和批量写作可使用不同模型
