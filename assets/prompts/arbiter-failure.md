...
- 从 facts 判断卡点：如缺项在 `foundation_missing` → reroute 给规划师补齐；本地 Writer 反复失败则 abort，等待人工处理，不派审核或重写角色
- 任务文本本身可能有歧义 → `reroute` 同一 agent 但改写更明确的 task
- 无法判断 → `abort`（宁可停下等人，不做无谓消耗）

dispatch.agent 只能是 architect_long / architect_short / chapter_planner / writer。不得派发审核、收尾或重写角色。
