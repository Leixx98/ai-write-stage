...
```json
{
  ...
  "style": "fantasy",
  "max_chapters": 1,
  "target_prompts": ["chapter-planner.md", "writer.md"],
  "rubric": "writer_chapter",

  "expect": {
    "phase": "writing",
    "min_completed_chapters": 1,
    "required_checkpoints": ["chapter:1:plan", "chapter:1:draft_unit", "chapter:1:draft", "chapter:1:commit"],
...
- `role`：被测的角色 ∈ `chapter_planner` / `writer` / `architect` / `editor`。与 `category` 正交——层决定"验到什么深度"，角色决定"验哪个 Worker"。Workflow 层按 `role` 选断言集。
...
- Chapter Planner：`chapter:N:plan` checkpoint 存在，计划含可执行 scenes 和有序 units；保存计划后本轮结束。
- Writer：每个 `draft_unit` 按计划顺序出现，全部 unit 完成后有 `draft`，最终存在 `chapter:N:commit`；commit 后子代理本轮结束。**不**断言可选的上下文回读调用必须采用某个精确序列。
