...
| Chapter Planner 已交接 | `chapter_planner-*.jsonl` 中有一次成功的 `plan_chapter`，计划含 scenes/units |
| Writer 没跳片段 | 每次 Writer run 只出现一个 `write_chapter_unit`；其返回的 `next_unit_id` 由下一次全新 run 接手，最后再单独出现 `check_consistency → commit_chapter` |
