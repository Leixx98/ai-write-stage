...
- **开放式创作 → 有界 LLM Worker**。全书/卷弧规划、章节执行规划、逐片段正文和评审分属独立 Worker；Engine 决定阶段顺序，Worker 只在本阶段边界内自主。
...
- **Checkpoint** — step 级推进记录（plan / draft_unit / draft / commit / review / arc_summary）
...
**铁律一：工具只返事实，不返跨调度指令**。`commit_chapter` 返回 `arc_end` / `needs_expansion` 等结构化字段；不夹带 `[系统]` 类指令字符串。子代理内的 `next_step` 字段只描述当前 Worker 内的后续操作（如继续写下一个 unit），不算跨调度指令——见 §6.3。
...
**铁律三：语义裁定走 Arbiter，每次裁定落盘**。启动选规划师、用户干预分诊、失败/僵局出路由 `internal/arbiter` 的逐场景 Decide 函数裁定：事实进、结构化决策出、机械校验兜底、decisions.jsonl 审计（可离线重放回归）。各 Worker 保留各自的 `CheckpointDeltaGuard`（事实护栏：产物未落盘不得收工）。
...
[architect_short/long · chapter_planner · writer · editor]（各自独立 run + context + 模型）
        │ 工具调用
[Tools]  novel_context · read_chapter · plan_chapter · write_chapter_unit · draft_chapter · edit_chapter
...
type Checkpoint struct {
    ...
    Step       string      // plan / draft_unit / draft / commit / review / arc_summary / ...
    Artifact   string
    Digest     string
...
| `write_chapter_unit` | drafts/chXX.units/NNN.md，并按顺序重建 chXX.draft.md | draft_unit（全部完成后追加 draft） |
...
`edit_chapter` 是 `agentcore.EditTool` 的薄封装，仅允许编辑已完成且位于 `PendingRewrites` 的章节。新式章节初稿必须按计划调用 `write_chapter_unit`；`draft_chapter(mode="write")` 只兼容旧版无 units 的计划、全部 units 完成后的整章修正和已完成章节返工。
...
`agents.BuildWorkers`（`internal/agents/build.go`）把四类 Worker 装配为一个 `subagent.Runner`：Engine 直接调用 `Run(agent, task)`，每次调用是一个完整的 `agentcore.AgentLoop`（独立 context、独立模型、独立重试）。全部装配一次生效：角色模型 + failover、prompt cache key（每 spawn 自增 #seq）、ThinkingLevel、UsageRecorder/SessionLogger（OnMessage）、Writer ContextManagerFactory（窗口随 /model 切换自动重建）、RestorePack、StopGuardFactory、StopAfterTools。
...
```
Engine ── Runner.Run(agent, task) ──▶ architect_short/long · chapter_planner · writer · editor
                                          │ 工具调用
                                        Store（协作媒介，Worker 之间不直接通信）
```

`bootstrap.ModelSet` 支持角色级模型：architect/chapter_planner/writer/editor 各自独立配置 + provider failover。chapter_planner 未显式配置时继承 architect，便于使用云端强模型规划、Ollama 本地模型逐片段写正文。Arbiter 统一使用 Default 模型（经 usageTrackedModel 计费），当前不开放独立角色配置。

### 6.2 三类协作模式

Worker 之间不直接通信，所有信息流经 Store 中的结构化工件：

**模式 A · 串行移交（主干）**：Route 派 Architect 规划全局结构 → Chapter Planner 规划当前章的 scenes/units → Writer 逐 unit 写正文 → Editor 弧末评审 → Writer 重写。每一步"下一个派谁"由 Route 从事实推导。
...
| 层 | 落点 | 作用 |
|---|---|---|
| `StopAfterTools` / `StopAfterToolResult` | `agents/build.go` SubAgentConfig | 关键工具成功即退出 Worker run（终态退出仍咨询 StopGuard，见契约测试）。Writer 每个 `write_chapter_unit` 或 `commit_chapter` 命中即停，下一 unit 由全新 run 执行；Editor 的 `save_review`/`save_arc_summary`/`save_volume_summary`、Architect 弧/卷收尾走 `StopAfterToolResult` |
...
internal/
  ...
  agents/         build.go 装配四类 Worker(subagent.Runner,Engine 程序化直调)；ctxpack/ Writer 上下文压缩策略
...
- 12 个单文件原子、跨文件显式失败/幂等恢复的工具；其中 commit 使用持久化 Saga + 一个 jsonl checkpoint 文件

模型升级的收益流向何处一目了然：规划更好（Architect/Chapter Planner）、创作更好（Writer）、评审更准（Editor）、裁定更准（Arbiter 四场景）、摘要更好（ctxpack）——全部换模型即得，外壳一行不改。控制面不吃模型红利，因为**查表不需要智力**；它需要的是被证明正确，而它已经被证明了。
