...
const maxToolProgressErrorBytes = 64 << 10
		...
		case agentcore.EventToolExecEnd:
			if opts.reportProgress {
				if ev.IsError {
					...
					var decoded string
					if json.Unmarshal(ev.Result, &decoded) == nil {
						errMsg = decoded
					}
					if len(errMsg) > maxToolProgressErrorBytes {
						cut := maxToolProgressErrorBytes
						for cut > 0 && !utf8.RuneStart(errMsg[cut]) {
							cut--
						}
						errMsg = errMsg[:cut] + "\n[tool error detail truncated at 64 KiB]"
					}
					agentcore.ReportToolProgress(ctx, agentcore.ProgressPayload{
