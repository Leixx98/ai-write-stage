...
func New(cfg Config) (*compat.Provider, error) {
	...
	return compat.New(cfg, compat.Spec{
		...
		Capabilities: func(_ string, caps litellm.Capabilities) litellm.Capabilities {
			caps.Thinking.Efforts = []string{"low", "high", "max"}
			caps.Thinking.BudgetTokens = litellm.SupportNo
			caps.Thinking.IncludeOutput = litellm.SupportNo
			caps.Thinking.Notes = []string{"minimal maps to low; medium maps to high; xhigh maps to max"}
			return caps
		},
	})
}
...
func thinkingEffort(value string) (string, error) {
	...
	case "xhigh":
		return "max", nil
	default:
		return "", fmt.Errorf("deepseek: unsupported thinking effort %q; use low, high, or max", value)
	}
}
...
func thinkingWarnings(req *litellm.Request) []litellm.Warning {
	...
	mapped, err := thinkingEffort(value)
	if err != nil || mapped == "" || mapped == value {
		return nil
	}
	return []litellm.Warning{{
		Code:     "deepseek.thinking_effort_folded",
		Provider: "deepseek",
		Message:  fmt.Sprintf("DeepSeek supports reasoning_effort low, high, or max; mapped %q to %s", value, mapped),
	}}
}
