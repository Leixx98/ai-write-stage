...
type writerStoreSummaryState struct {
	...
	writingProgress   *domain.WritingProgress
	chapter           int
...
func loadWriterStoreSummaryState(s *store.Store) (*writerStoreSummaryState, bool, error) {
	...
	state.writingProgress, err = s.Drafts.LoadWritingProgress(chapter)
	if err != nil {
		state.warn("writing_progress", err)
	}
	...
	hasWorkingState := state.chapterPlan != nil || state.currentOutline != nil || state.writingProgress != nil
	if !hasWorkingState {
		return nil, false, nil
	}
...
func writerStoreProgressSection(state *writerStoreSummaryState) map[string]any {
	...
	if state.writingProgress != nil {
		section["writing_progress"] = state.writingProgress
	}
	return section
}
