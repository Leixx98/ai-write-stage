...
func (p *WriterRestorePack) buildMessage(budgetTokens int) (agentcore.Message, bool, error) {
	...
	// Small local windows cannot fit the 6K restore ceiling. Fit a leading
	// projection instead of failing the entire compaction pass; builder order
	// deliberately places current progress and the active unit first.
	runes := []rune(p.text)
	const marker = "\n[恢复包已按本地模型窗口裁剪]"
	low, high, best := 0, len(runes), ""
	for low <= high {
		mid := low + (high-low)/2
		candidate := string(runes[:mid]) + marker
