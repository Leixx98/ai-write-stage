...
func TestWriterStoreSummarySupportsFirstChapterWithoutPriorSummaries(t *testing.T) {
	...
	if err := s.Drafts.SaveChapterPlan(domain.ChapterPlan{
		...
		Scenes: []domain.ScenePlan{{
			...
			Units: []domain.WritingUnit{{
				ID:            "1-1-1",
				TargetChars:   900,
				RequiredBeats: []string{"主角发现第一条线索"},
				EndAnchor:     "门外传来脚步声",
			}},
		}},
	}); err != nil {
		t.Fatalf("Save chapter plan: %v", err)
	}

	text, ok, err := buildWriterStoreSummaryText(s, 2000)
	if err != nil {
		t.Fatalf("buildWriterStoreSummaryText: %v", err)
	}
	if !ok {
		t.Fatal("first chapter plan should support deterministic store compaction")
	}
	if !strings.Contains(text, `"writing_progress"`) || !strings.Contains(text, `"id":"1-1-1"`) {
		t.Fatalf("summary must preserve the exact current writing unit: %q", text)
	}
}
...
func TestWriterRestorePackRefreshReusesStoreBuilder(t *testing.T) {
	...
	if _, ok, err := pack.buildMessage(0); err != nil || ok {
		t.Fatalf("zero room should skip restore pack, ok=%v err=%v", ok, err)
	}
	compact, ok, err := pack.buildMessage(200)
	if err != nil || !ok {
		t.Fatalf("small local window should receive a trimmed restore pack, ok=%v err=%v", ok, err)
	}
	if got := corecontext.EstimateTokens(compact); got > 200 {
		t.Fatalf("trimmed restore pack exceeds budget: %d", got)
	}
	if !strings.Contains(compact.TextContent(), "恢复包已按本地模型窗口裁剪") {
		t.Fatal("trimmed restore pack should disclose projection")
	}
}
