...
func (t *ContextTool) Schema() map[string]any {
	return schema.Object(
		...
		schema.Property("context_mode", schema.Enum("上下文档位；本地逐片段写作用 writer_unit，云端章节规划用 chapter_plan", "full", "chapter_plan", "writer_unit")),
	)
}

func (t *ContextTool) Execute(ctx context.Context, args json.RawMessage) (json.RawMessage, error) {
	return t.execute(ctx, args, 36*1024)
}

func (t *ContextTool) execute(_ context.Context, args json.RawMessage, writerUnitBudget int) (json.RawMessage, error) {
	var a struct {
	...
	if a.Chapter > 0 {
		...
		t.buildWritingProgress(result, a.Chapter, warn)
	...
	if a.Chapter > 0 {
		...
		case "writer_unit":
			trimWriterUnitContext(result, writerUnitBudget)
		case "chapter_plan":
			trimByBudget(result, 60*1024)
		default:
			trimByBudget(result, 100*1024)
		}
	} else {
		trimByBudget(result, 60*1024) // Architect: 60KB
	}

	if _, projected := result["_loading_summary"]; !projected {
		result["_loading_summary"] = buildLoadingSummary(result, a.Chapter)
	}
	return json.Marshal(result)
}

// buildWritingProgress 注入由 unit 工件推导的细粒度进度。它只出现在采用场景卡
// 的新计划中；旧计划保持原有上下文形状并继续走 legacy 写作路径。
func (t *ContextTool) buildWritingProgress(result map[string]any, chapter int, warn func(string, error)) {
	progress, err := t.store.Drafts.LoadWritingProgress(chapter)
	if err != nil {
		warn("writing_progress", err)
		return
	}
	...
	working["writing_progress"] = progress
}
...
func trimByBudget(result map[string]any, budget int) {
	...
	trimContextByOrder(result, budget, trimOrder)
}

// trimWriterUnitContext 为本地逐片段写作保留当前章/当前 unit 的执行事实，优先
// 去掉可由 current_chapter_outline、chapter_plan 和 selected_memory 替代的全书镜像。
// previous_tail 不参与此档位裁剪，否则首个 unit 会失去与上一章的直接衔接。
func trimWriterUnitContext(result map[string]any, budget int) {
	if projectWriterUnitContext(result, budget) {
		return
	}

	// Legacy plans without writing units keep the older chapter-level shape.
	// 这三项是规划期的全局镜像；Writer 已有当前章大纲和 chapter_plan，始终移除。
	var projected []string
	for _, key := range []string{"outline", "premise_sections", "premise_structure"} {
		if _, ok := result[key]; ok {
			deleteContextKey(result, key)
			projected = append(projected, key)
		}
	}
	appendTrimmedKeys(result, projected)
...
func trimContextByOrder(result map[string]any, budget int, trimOrder []string) {
	...
	if len(trimmed) > 0 {
		appendTrimmedKeys(result, trimmed)
	}
}

func appendTrimmedKeys(result map[string]any, keys []string) {
	if len(keys) == 0 {
		return
	}
	if existing, ok := result["_trimmed"].([]string); ok {
		keys = append(existing, keys...)
	}
	result["_trimmed"] = keys
}
