...
func (t *CheckConsistencyTool) Description() string {
	return "加载已完成的整章草稿和对照数据（世界规则、伏笔、关系、别名、最近摘要），供你检查一致性。新计划必须先完成全部 writing units"
}
...
func (t *CheckConsistencyTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	if !completed {
		...
		if unitProgress != nil && unitProgress.TotalUnits > 0 && !unitProgress.Complete {
			return nil, fmt.Errorf("writing units incomplete (%d/%d); finish write_chapter_unit before chapter consistency check: %w",
