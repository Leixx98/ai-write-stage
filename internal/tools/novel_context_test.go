...
func TestTrimWriterUnitContextDropsGlobalOutlineAndKeepsTail(t *testing.T) {
	result := map[string]any{
		"outline":       strings.Repeat("全书大纲", 200),
		"references":    strings.Repeat("参考资料", 200),
		"previous_tail": "必须保留的上一章结尾",
		"working_memory": map[string]any{
			"previous_tail":    "必须保留的上一章结尾",
			"chapter_plan":     map[string]any{"chapter": 8},
			"writing_progress": map[string]any{"next": map[string]any{"unit_id": "8-1-1"}},
		},
	}

	trimWriterUnitContext(result, 300)

	if _, ok := result["outline"]; ok {
		t.Fatal("writer_unit context should not retain the full-book outline")
	}
	if _, ok := result["previous_tail"]; !ok {
		t.Fatal("writer_unit context must retain previous_tail")
	}
	working := result["working_memory"].(map[string]any)
	for _, key := range []string{"previous_tail", "chapter_plan", "writing_progress"} {
		if _, ok := working[key]; !ok {
			t.Fatalf("writer_unit context lost required key %q", key)
		}
	}
}
