...
func (t *DraftChapterTool) Description() string {
	return "整章草稿工具，仅用于旧版无 units 的计划、全部 units 完成后的整章修正或已完成章节返工；新计划初稿必须用 write_chapter_unit"
}
...
func (t *DraftChapterTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	if completed {
		...
		if loadErr != nil {
			return nil, fmt.Errorf("load writing progress: %w: %w", errs.ErrStoreRead, loadErr)
		}
		if unitProgress != nil && unitProgress.TotalUnits > 0 && !unitProgress.Complete {
			return nil, fmt.Errorf("新章节仍有 writing unit 未完成（%d/%d）；必须继续调用 write_chapter_unit，不能用 draft_chapter 绕过片段计划: %w",
				unitProgress.CompletedUnits, unitProgress.TotalUnits, errs.ErrToolPrecondition)
		}
	}
