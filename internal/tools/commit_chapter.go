...
func (t *CommitChapterTool) Description() string {
	return "提交章节终稿。加载草稿正文保存为终稿，更新时间线、伏笔、关系、角色状态和进度。" +
		"返回结构化事实：next_chapter / arc_end / volume_end / needs_expansion / book_complete / flow 等"
}
...
func (t *CommitChapterTool) Execute(_ context.Context, args json.RawMessage) (json.RawMessage, error) {
	...
	if !completed {
		unitProgress, loadErr := t.store.Drafts.LoadWritingProgress(a.Chapter)
		if loadErr != nil {
			return nil, fmt.Errorf("load writing unit progress: %w: %w", errs.ErrStoreRead, loadErr)
		}
		if unitProgress != nil && unitProgress.TotalUnits > 0 && !unitProgress.Complete {
			nextID := "<unknown>"
			if unitProgress.Next != nil {
				nextID = unitProgress.Next.Unit.ID
			}
			return nil, fmt.Errorf("章节写作片段尚未完成（%d/%d，下一片段 %s），不能提交: %w",
				unitProgress.CompletedUnits, unitProgress.TotalUnits, nextID, errs.ErrToolPrecondition)
		}
	}
	if existingPending != nil && (existingPending.Stage == domain.CommitStageProgressMarked || existingPending.Stage == domain.CommitStageSignalSaved) {
		if !completed {
...
// ── 分层完结判定 ──
//
// 完结检查发生在末章自动提交时。已宣告收官的分层小说只要求结构写完，
// 不再等待 Editor 评审、弧摘要或卷摘要。
...
//   - 已宣告收官卷（layered_outline 最后一卷带 final）→ 结构写完即完结。
...
func layeredComplete(st *store.Store, progress *domain.Progress) (bool, error) {
	...
	if domain.FinaleVolume(volumes) > 0 {
		return layeredStructurallyComplete(st, progress)
	}
