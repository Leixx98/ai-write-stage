...
type engine struct {
	...
	autoCommit func(context.Context, int) error // units 完成后由程序提交，不经过 LLM
...
func (e *engine) run(ctx context.Context) {
	...
	for {
		...
		if inst == nil {
			...
			if chapter := automaticCommitChapter(state); chapter > 0 {
				if e.autoCommit == nil {
...
func automaticCommitChapter(state flow.State) int {
	if state.Progress == nil || state.Progress.Phase != domain.PhaseWriting || len(state.Progress.PendingRewrites) > 0 {
		return 0
	}
	writing := state.NextChapterWriting
...
func (e *engine) precheck(inst *flow.Instruction) (*flow.Instruction, error) {
	...
	if inst.Agent == "chapter_planner" {
		if progress == nil || progress.Phase != domain.PhaseWriting {
			phase := "<nil>"
			if progress != nil {
				phase = string(progress.Phase)
			}
			return nil, fmt.Errorf("chapter_planner 仅能在 writing 阶段派发（当前 phase=%s）: %w", phase, errInvalidWriteTarget)
		}
