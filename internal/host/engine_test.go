...
var plannedChapterRe = regexp.MustCompile(`第 (\d+) 章`)
var plannedUnitRe = regexp.MustCompile(`unit ([^（\s]+)`)

// scriptedWriterModel 对新章只写任务指定的一个 unit；Engine 会为下一 unit
// 创建新 Worker 会话，并在全部 units 完成后自动提交。返工保留旧兼容流程。
	...
	return &scriptedChatModel{fn: func(msgs []agentcore.Message) agentcore.Message {
		...
		for _, m := range msgs {
			if m.Role == agentcore.RoleUser {
				text := m.TextContent()
				if match := plannedChapterRe.FindStringSubmatch(text); match != nil {
					chapter, _ = strconv.Atoi(match[1])
				}
				if match := plannedUnitRe.FindStringSubmatch(text); match != nil {
					unitID = match[1]
				}
				rewrite = rewrite || strings.Contains(text, "重写第") || strings.Contains(text, "打磨第")
			}
		...
		if rewrite {
			...
			case 0:
				return testToolCallMsg("draft_chapter", map[string]any{
					...
					"content": strings.Repeat(fmt.Sprintf("第%d章的返工正文，主角重新审视眼前的局势。", chapter), 24),
				})
			case 1:
				return testToolCallMsg("check_consistency", map[string]any{"chapter": chapter})
			default:
				return scriptedCommitMessage(chapter)
			}
		}
		if unitID == "" {
			unitID = fmt.Sprintf("%d-1-1", chapter)
		}
		return testToolCallMsg("write_chapter_unit", map[string]any{
			"chapter": chapter, "unit_id": unitID,
			"content": strings.Repeat(fmt.Sprintf("第%d章片段%s的正文，主角在黑暗中摸索，线索逐渐指向更深处。", chapter, unitID), 20),
		})
	}}
}

func scriptedCommitMessage(chapter int) agentcore.Message {
...
func scriptedChapterPlannerModel() *scriptedChatModel {
	return &scriptedChatModel{fn: func(msgs []agentcore.Message) agentcore.Message {
		chapter := 0
		for _, m := range msgs {
			if m.Role != agentcore.RoleUser {
			...
			if match := plannedChapterRe.FindStringSubmatch(m.TextContent()); match != nil {
				chapter, _ = strconv.Atoi(match[1])
			}
		}
		return testToolCallMsg("plan_chapter", testChapterPlanArgs(chapter))
	}}
}

func testChapterPlanArgs(chapter int) map[string]any {
	return map[string]any{
		"chapter": chapter, "title": fmt.Sprintf("第%d章", chapter),
		"goal": "推进主线", "conflict": "主角遇阻", "hook": "悬念收尾",
		"opening_state": "主角进入黑暗走廊", "target_chars": 1600,
		"creative_freedom": []string{"对话措辞和环境细节"},
		"scenes": []map[string]any{{
			"id": fmt.Sprintf("%d-1", chapter), "purpose": "发现并确认线索",
			"location": "黑暗走廊", "pov": "主角", "characters": []string{"主角"},
			"entry_state": "尚未发现线索", "conflict": "视野受阻", "turn": "发现暗门",
			"exit_state": "确认暗门通向深处", "target_chars": 1600,
			"units": []map[string]any{
				{"id": fmt.Sprintf("%d-1-1", chapter), "target_chars": 800, "required_beats": []string{"摸索走廊", "发现痕迹"}, "forbidden_moves": []string{"打开暗门"}, "end_anchor": "确认墙上有缝隙", "transition": "从进入走廊开始"},
				{"id": fmt.Sprintf("%d-1-2", chapter), "target_chars": 800, "required_beats": []string{"确认暗门", "听见门后动静"}, "forbidden_moves": []string{}, "end_anchor": "暗门后传来脚步", "transition": "承接墙缝继续调查"},
			},
		}},
		"required_beats": []string{"发现暗门"}, "forbidden_moves": []string{},
		"continuity_checks": []string{"主角尚不知道门后是谁"}, "evaluation_focus": []string{"推进是否自然"},
	}
}

func testChapterPlanner(st *storepkg.Store) subagent.Config {
	return subagent.Config{
		Name: "chapter_planner", Description: "test chapter planner", Model: scriptedChapterPlannerModel(),
		SystemPrompt: "test", Tools: []agentcore.Tool{tools.NewPlanChapterTool(st)},
		MaxTurns: 3, StopAfterTools: []string{"plan_chapter"},
	}
}

func saveTestChapterPlan(t *testing.T, st *storepkg.Store, chapter int) {
	t.Helper()
	plan := domain.ChapterPlan{
		Chapter: chapter, Title: fmt.Sprintf("第%d章", chapter), Goal: "推进主线",
		Conflict: "主角遇阻", Hook: "悬念收尾", OpeningState: "主角进入黑暗走廊", TargetChars: 1600,
		CreativeFreedom: []string{"对话措辞和环境细节"},
		Scenes: []domain.ScenePlan{{
			ID: fmt.Sprintf("%d-1", chapter), Purpose: "发现并确认线索", Location: "黑暗走廊", POV: "主角",
			Characters: []string{"主角"}, EntryState: "尚未发现线索", Conflict: "视野受阻", Turn: "发现暗门",
			ExitState: "确认暗门通向深处", TargetChars: 1600,
			Units: []domain.WritingUnit{
				{ID: fmt.Sprintf("%d-1-1", chapter), TargetChars: 800, RequiredBeats: []string{"摸索走廊", "发现痕迹"}, ForbiddenMoves: []string{"打开暗门"}, EndAnchor: "确认墙上有缝隙", Transition: "从进入走廊开始"},
				{ID: fmt.Sprintf("%d-1-2", chapter), TargetChars: 800, RequiredBeats: []string{"确认暗门", "听见门后动静"}, EndAnchor: "暗门后传来脚步", Transition: "承接墙缝继续调查"},
			},
		}},
	}
...
func newTestEngine(t *testing.T, st *storepkg.Store, workers *subagent.Runner, arbiterModel agentcore.ChatModel) (*engine, *[]Event, chan struct{}) {
	...
	e := &engine{
		...
		autoCommit: func(ctx context.Context, chapter int) error {
			_, err := tools.AutoCommitPlannedChapter(ctx, st, tools.NewStyleStatsIndex(st), chapter)
			return err
		},
...
func TestEngine_ReviewPermitWritesExactlyOneNewChapter(t *testing.T) {
	...
	writer := subagent.Config{
		...
		Tools: []agentcore.Tool{
			tools.NewWriteChapterUnitTool(st), tools.NewDraftChapterTool(st),
			tools.NewCheckConsistencyTool(st), tools.NewCommitChapterTool(st, tools.NewStyleStatsIndex(st)),
		},
		MaxTurns: 10, StopAfterTools: []string{"write_chapter_unit", "commit_chapter"},
	}
	e, _, done := newTestEngine(t, st, subagent.NewRunner(testChapterPlanner(st), writer), nil)
...
func TestEngine_WritesBookToCompletion(t *testing.T) {
	...
	writer := subagent.Config{
		...
		SystemPrompt: "test",
		Tools: []agentcore.Tool{
			tools.NewWriteChapterUnitTool(st),
			tools.NewDraftChapterTool(st),
			tools.NewCheckConsistencyTool(st),
			tools.NewCommitChapterTool(st, tools.NewStyleStatsIndex(st)),
		},
		MaxTurns:       10,
		StopAfterTools: []string{"write_chapter_unit", "commit_chapter"},
	}
	e, events, done := newTestEngine(t, st, subagent.NewRunner(testChapterPlanner(st), writer), nil)
	...
	var dispatches, toolRows, automaticCommits int
	...
	if automaticCommits != 2 {
		t.Fatalf("每章必须由 Engine 直接提交且不派 Finalizer, got %d", automaticCommits)
	}
}
