...
// MinCompactReserve 是 ReserveTokens 的目标下限。小窗口模型（如 32k 本地 qwen3:8b）
// 按 0.15 比例算 reserve 仅 4800，单次 commit_chapter 工具响应就能塞 5-8k，
// 一章正文 8-15k——会出现"压完立刻又超"。8000 兜底保证最坏场景下还有半轮缓冲。
// 当窗口本身小于 16k 时，reserve 最多占一半；否则 8k 窗口会只剩 192 tokens
// 的荒谬阈值，压缩器从首条消息起就永远处于压力状态。
...
//	reserve   = min(window / 2, max(MinCompactReserve, window * (1 - CompactRatio)))
//
...
func CompactReserveTokens(window int) int {
	...
	if reserve < MinCompactReserve {
		reserve = MinCompactReserve
	}
	if maxReserve := window / 2; reserve > maxReserve {
		return maxReserve
	}
	return reserve
}
...
func (c *Config) ValidateBase() error {
	...
	for role, rc := range c.Roles {
		...
		if !knownRoles[role] {
			return fmt.Errorf("unknown role %q in roles config (valid: architect/chapter_planner/writer/editor/prompter/galgame/import_segment/import_analyze/import_synthesize): %w", role, errs.ErrConfig)
		}
...
// chapter_planner 未显式配置时随 architect 模型，也继承 architect 的推理强度。
...
func (c Config) ResolveReasoningEffort(role string) string {
	if role != "" && role != "default" {
		...
		if role == "chapter_planner" {
			if rc, ok := c.Roles["architect"]; ok && rc.ReasoningEffort != "" {
				return rc.ReasoningEffort
			}
		}
	}
