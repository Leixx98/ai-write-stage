...
func TestConfigResolveReasoningEffort(t *testing.T) {
	...
	inherited := Config{
		ReasoningEffort: "low",
		Roles: map[string]RoleConfig{
			"architect": {ReasoningEffort: "high"},
		},
	}
	if got := inherited.ResolveReasoningEffort("chapter_planner"); got != "high" {
		t.Errorf("未显式配置的 chapter_planner 应继承 architect，得 %q", got)
	}
	inherited.Roles["chapter_planner"] = RoleConfig{ReasoningEffort: "medium"}
	if got := inherited.ResolveReasoningEffort("chapter_planner"); got != "medium" {
		t.Errorf("chapter_planner 显式覆盖应优先，得 %q", got)
	}
}
