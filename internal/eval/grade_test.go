...
func writerSmokeCase() Case {
	c := Case{
		...
		Expect: Expect{
			...
			RequiredCheckpoints:  []string{"chapter:1:plan", "chapter:1:draft_unit", "chapter:1:draft", "chapter:1:commit"},
...
func cleanCollected() Collected {
	return Collected{
		...
		Checkpoints: []domain.Checkpoint{
			...
			{Scope: domain.ChapterScope(1), Step: "draft_unit"},
...
func TestGradeCatchesMissingCommit(t *testing.T) {
	col := cleanCollected()
	col.Checkpoints = col.Checkpoints[:3] // 去掉 commit
...
func TestParseCheckpointSpec(t *testing.T) {
	cases := []struct {
		...
		{"chapter:1:draft_unit", domain.ScopeChapter, "draft_unit", true},
