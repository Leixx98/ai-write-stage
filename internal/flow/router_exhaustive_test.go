...
//  4. Flow=Steering → LLM 裁定（nil）；旧 Reviewing 按普通 writing 恢复
//  5. 分层模式弧末 → 展开下一弧 / 追加新卷
//  6. 下一章计划缺失 → chapter_planner
//  7. 有待写 unit → writer；units 全部完成 → nil，由 Engine 自动提交
func expectedInstruction(s State) expectKind {
	...
	if !s.HasNextChapterPlan {
		return expectChapterPlan
	}
	if s.NextChapterWriting != nil && s.NextChapterWriting.TotalUnits > 0 && s.NextChapterWriting.Complete {
		return expectNil
	}
	if s.NextChapterWriting != nil && s.NextChapterWriting.TotalUnits > 0 && s.NextChapterWriting.Next != nil {
...
func enumerateBoundaryCases() []boundaryCase {
	return []boundaryCase{
		...
		{name: "settled-arc", boundary: &storepkg.ArcBoundary{IsArcEnd: true, Volume: 2, Arc: 3}},
		{name: "expand", boundary: &storepkg.ArcBoundary{IsArcEnd: true, Volume: 2, Arc: 3, NextVolume: 2, NextArc: 4, NeedsExpansion: true}},
		{name: "expand-no-nextarc", boundary: &storepkg.ArcBoundary{IsArcEnd: true, Volume: 2, Arc: 3, NeedsExpansion: true}},
		{name: "new-volume", boundary: &storepkg.ArcBoundary{IsArcEnd: true, IsVolumeEnd: true, Volume: 2, Arc: 3, NeedsNewVolume: true}},
	}
}
...
func TestRoute_ExhaustiveAgainstSpec(t *testing.T) {
	...
	chapterPlans := []bool{false, true}
	writingStates := []*domain.WritingProgress{
		nil,
		{Chapter: 1, TotalUnits: 2, Next: &domain.WritingUnitAssignment{Ordinal: 1, Unit: domain.WritingUnit{ID: "unit-a"}}},
		{Chapter: 1, TotalUnits: 2, CompletedUnits: 2, Complete: true},
	}
					...
					for _, completed := range completedSets {
						for _, missing := range missingSets {
							for _, tier := range tiers {
								for _, hasPlan := range chapterPlans {
									for _, writing := range writingStates {
										for _, bc := range enumerateBoundaryCases() {
											...
											s := State{
												...
												PlanningTier:       tier,
												HasNextChapterPlan: hasPlan,
												NextChapterWriting: writing,
											}
											...
											got := classify(t, inst)
											if got != want {
												t.Fatalf("phase=%s flow=%s queue=%v layered=%v completed=%v missing=%v tier=%q plan=%v writing=%+v boundary=%s:\n规格期望 %d，实现返回 %d（inst=%+v）",
													phase, fl, queue, layered, completed, missing, tier, hasPlan, writing, bc.name, want, got, inst)
											}
...
func assertConservation(t *testing.T, s State, inst *Instruction) {
	...
	switch inst.Agent {
	case "chapter_planner":
		if inst.Chapter != p.NextChapter() || !contains(inst.Task, "plan_chapter") {
			t.Fatalf("chapter_planner 必须规划下一章并明确落盘工具：%+v", inst)
		}
	case "writer":
		if inst.Chapter <= 0 {
...
func snapshotState(s State) State {
	...
	if s.NextChapterWriting != nil {
		writing := *s.NextChapterWriting
		writing.CompletedUnitIDs = append([]string(nil), s.NextChapterWriting.CompletedUnitIDs...)
		if s.NextChapterWriting.Next != nil {
			next := *s.NextChapterWriting.Next
			next.Characters = append([]string(nil), s.NextChapterWriting.Next.Characters...)
			next.Unit.RequiredBeats = append([]string(nil), s.NextChapterWriting.Next.Unit.RequiredBeats...)
			next.Unit.ForbiddenMoves = append([]string(nil), s.NextChapterWriting.Next.Unit.ForbiddenMoves...)
			writing.Next = &next
		}
		cp.NextChapterWriting = &writing
	}
